...
# Setting up constats
NUM_WORMS = 1  # the number of worms in the grid
FPS = 60        # frames per second that the program runs
CELLS_SIZE = 20 # how many pixels wide and high each "cell" in the grid is
CELLS_WIDE = 20 # how many cells wide the grid is
CELLS_HIGH = 20 # how many cells high the grid is
...
class Worm(threading.Thread): # "Thread" is a class in the "threading" module.
    ...
    def run(self):
        while True:
            ...
            (theNextPosition, cost) = self.getNextPosition()
            #print(cost)
            if theNextPosition is not None and self.JudgeSafe(FOOD, cost) is True:
                print(theNextPosition)
                print("True")
                (nextx, nexty) = theNextPosition
            else:
                print(theNextPosition)
                print("False")
                theNextPosition = self.Mode2()
                print("Mode2")
                print(theNextPosition)
    ...
    def BFS(self, start, end, visited):
        ...
        #print(end["current"])
        queList = []
        queList.append(start)

        while len(queList) != 0:
            #print("\n")
            current = queList.pop(-1)
            ...
            for step in STEPS:
                next = self.NextNodeNJudge(step, current, end)
    ...
    def JudgeSafe(self, position, cost):
       if cost+1 > len(self.body):
          cost = 0
       ...
       endNode = {
           'current' : (self.body[BUTT]['x'], self.body[BUTT]['y']),
           'value_f' : 0,
           'value_h' : 0,
           'value_g' : 0,
           'parent'  : None
       ...
       visited = [[False for i in range(CELLS_WIDE)] for j in range(CELLS_HIGH)]

       return self.BFS(startNode, endNode, visited)
    ...
    def Mode2(self):
        ...
        langList = []
        tailDirect = (self.body[BUTT-1]['x']-self.body[BUTT]['x'], self.body[BUTT-1]['y']-self.body[BUTT]['y'])
        print(tailDirect)
        for step in STEPS:
            next = self.NextNode(step, startNode, endNode)
            if next != None:
                langList.append(next)

        print("Mode2 start")
        while langList != None and len(langList) != 0:
            #langList.sort(key=lambda node : node["value_h"])
            current = langList.pop(-1)
            print(current["current"])
            if self.JudgeSafe(current["current"], 1) is True:
    ...
    def getNextPosition(self):
        ...
        if result["parent"] == None:
            ...
            cost = endNode['parent']['value_g']
        return result["current"], cost


    def NextNodeNJudge(self, step, parent, end):
        currentNode = {
        ...
        if 0 <= currentNode['current'][0] < CELLS_WIDE and \
           0 <= currentNode['current'][1] < CELLS_HIGH:
            if GRID[currentNode['current'][0]][currentNode['current'][1]] == None or \
               GRID[currentNode['current'][0]][currentNode['current'][1]] == WHITE or \
               currentNode['current'] == (self.body[BUTT]['x'], self.body[BUTT]['y']):
                   return currentNode
        else:
            return None
...
def drawGrid():
    ...
    for x in range(0, CELLS_WIDE):
        for y in range(0, CELLS_HIGH):
            ...
            pygame.draw.rect(DISPLAYSURF, color, (x * CELLS_SIZE + 4, y * CELLS_SIZE + 4, CELLS_SIZE - 8, CELLS_SIZE - 8))
